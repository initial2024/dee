...
try{
  ...
  if(-not $explain.delegation_allowed){$summary=$explain.why}
    ...
    $job=Start-Job -ScriptBlock { param($prompt,$budget) & xiaoyu-router delegate-fast --max-seconds $budget $prompt 2>&1 } -ArgumentList $safeTask,$MaxSeconds
    if(Wait-Job -Job $job -Timeout ($MaxSeconds + 3)){$result=Receive-Job -Job $job; Remove-Job -Job $job -Force; try{$routerResult=($result|Out-String)|ConvertFrom-Json;$success=[bool]$routerResult.ok;$provider=$routerResult.provider;$model=$routerResult.model;$summary=[string]$routerResult.summary;$errorCode=$routerResult.error_code}catch{$summary='Router returned an invalid delegation response.';$errorCode='ROUTER_RESPONSE_INVALID'}}
