...
function Get-ProviderRows {
    ...
    foreach ($property in @($data.providers.psobject.Properties)) {
        ...
        $discovered = @($provider.last_discovery_models); if($discovered.Count -eq 0){$discovered=@($provider.models)}; $seeds=@($provider.allowed_model_seeds); $usable = if($seeds.Count -gt 0){@($discovered | Where-Object { $_ -in $seeds })}else{$discovered}
...
Add-ProviderButton '详情' { $id = Require-SelectedProvider; if ($id) { $provider = (Get-ProviderData).providers.($id); $headerNames = if ($provider.headers) { $provider.headers.psobject.Properties.Name -join ', ' } else { 'NONE' }; $keyConfigured = if ($provider.api_key_env -and [Environment]::GetEnvironmentVariable($provider.api_key_env,'User')) { 'YES' } else { 'NO' }; $discovered = if($provider.last_discovery_models){@($provider.last_discovery_models) -join ', '}elseif($provider.models){@($provider.models) -join ', '}else{'NONE'}; $seeds=if($provider.allowed_model_seeds){@($provider.allowed_model_seeds) -join ', '}else{'NONE'}; $runtimeState=(Read-JsonFile $RuntimeConfig ([pscustomobject]@{providers=[pscustomobject]@{}})).providers.($id); $responsive=if($runtimeState){@($runtimeState.psobject.Properties|Where-Object{$_.Value.status -eq 'PASS'}|ForEach-Object{$_.Name}) -join ', '}else{'NONE'}; $cooldown=if($runtimeState){@($runtimeState.psobject.Properties|Where-Object{$_.Value.status -eq 'TIMEOUT'}|ForEach-Object{$_.Name}) -join ', '}else{'NONE'}; $runtime = @((Get-ProviderRows)|Where-Object{$_.provider_id -eq $id}|Select-Object -First 1); $runtimeStatus = if($runtime.Count){$runtime[0].last_runtime_status}else{'UNKNOWN'}; [System.Windows.Forms.MessageBox]::Show(("Provider ID：{0}`r`n显示名：{1}`r`nBase URL：{2}`r`nWire API：{3}`r`n启用：{4}`r`n发现认证：{5}`r`n推理认证：{6}`r`nAPI Key 已配置：{7}`r`nAPI Key 环境变量：{8}`r`nHeader 名称：{9}`r`n发现模型：{10}`r`n允许种子：{11}`r`n拒绝模型：按策略过滤`r`n运行可用模型：{12}`r`n超时冷却模型：{13}`r`n最近发现状态：{14}`r`n最近运行状态：{15}" -f $id,$provider.display_name,$provider.base_url,$provider.wire_api,$provider.enabled,$provider.model_discovery_auth_style,$provider.inference_auth_style,$keyConfigured,$provider.api_key_env,$headerNames,$discovered,$seeds,$responsive,$cooldown,$provider.last_discovery_status,$runtimeStatus),'供应商详情'); Add-ProviderLog ('查看详情：' + $id) } }
