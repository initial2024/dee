...
## Bonsai runtime layer

The local backend has two distinct runtime slots: `standard_llama_cpp` keeps using the existing ordinary-GGUF `llama-server`; `prism_bonsai` is reserved for Bonsai/Ternary-Bonsai formats. Run `xiaoyu-router local bonsai-runtime status` to inspect the dedicated slot. A missing or unverified runtime reports `BONSAI_RUNTIME_UNKNOWN` and blocks model download. `scripts\\install-bonsai-runtime.ps1` defaults to detection only; it never builds, never downloads a model, and uses only the official PrismML repositories if run with its explicit installation switch. `PQ2_0`, `PTQ1_0`, and Bonsai `Q1_0` require the Prism slot; `Q2_0_g64` uses standard llama.cpp only when that runtime explicitly advertises it. `mmproj` is never a text model. Bonsai remains excluded from automatic selection until a dedicated runtime and model smoke pass.
