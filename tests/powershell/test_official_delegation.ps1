...
$instructionText = Get-Content -LiteralPath $instruction -Raw -Encoding UTF8
...
Assert-True ($instructionText -match 'high-risk work remains under official Codex control') 'instructions_keep_high_risk_official'
Assert-True ($instructionText -match '(?i)do not copy official internal cache') 'instructions_make_no_cache_copy_claim'
